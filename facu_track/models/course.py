...
class Course(models.Model):
    ...
    student_ids = fields.One2many('facu_track.student', 'course_id', string='Students', readonly=True,required=True)
    ...
    def access_location(self):
        ...
        if self.check_lecture_time() == 'before':
            raise ValidationError("The time of the lecture didn't start yet")

        if self.check_lecture_time() == 'after':
            raise ValidationError("Sorry the time of lecture has finished")
    ...
    def check_lecture_time(self):
        date_time_now = datetime.now() + timedelta(hours=3)
        if date_time_now < self.begining_time:
            return 'before'
        elif date_time_now > self.end_time:
            return 'after'
        else:
            return True
...
#         users = self.search([('id', 'in',[242,243,244,345,246,247])])
